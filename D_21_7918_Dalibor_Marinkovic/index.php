    ...
    <div class="wrapper main">
        ...
        if($tabela==='filmovi'){
            echo '<div class="second-form"><form action="index.php"  method="POST">
                ...
                <input type="text" name="reziseri_id"><br><br>
                <input type="submit" value="Submit">
            </form></div>';
        }
        if((isset($_POST['id'])&& $_POST['id'] !='') && (isset($_POST['naslov']) && $_POST['naslov'])){
            $id = $conn->real_escape_string($_POST['id']);
            ...
            $reziserId = $conn->real_escape_string($_POST['reziseri_id']);
            $sql="INSERT INTO filmovi (id, naslov, godina, ocena, reziseri_id) VALUES ('".$id."','".$naslov."','".$godina."', '".$ocena."', '".$reziserId."')";
            if(!$result = $conn->query($sql)){
                die('There was an error running the query [' . $conn->error . ']');
            }
            else{
        ...
        if($tabela==='filmovi_zanr'){
            echo '<div class="second-form"><form action="index.php"  method="POST">
                <label for="filmovi_zanr">Filmo id: </label>
                <input type="number" name="filmovi_id"><br><br>
                <label for="filmovi_zanr">Zanr id: </label>
                <input type="text" name="zanrovi_id"><br><br>
                <input type="submit" value="Submit">
            </form></div>';
        }
        if((isset($_POST['filmovi_id'])&& $_POST['filmovi_id'] !='') && (isset($_POST['zanrovi_id']) && $_POST['zanrovi_id'])){
            $film_id = $conn->real_escape_string($_POST['filmovi_id']);
            $zanr_id = $conn->real_escape_string($_POST['zanrovi_id']);
            $sql="INSERT INTO filmovi_zanr (filmovi_id,zanrovi_id) VALUES('".$film_id."','".$zanr_id."')";
            if(!$result = $conn->query($sql)){
                die('There was an error running the query [' . $conn->error . ']');
            }
            else{
